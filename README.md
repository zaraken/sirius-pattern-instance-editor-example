...
<img src="doc/img/patternLayerSpec.jpg" /> <br/>
<img src="doc/img/patternLayerDiagram.jpg" /> <br/>

### pdiInstantiation

Imported mappings are named with "imp" in the name. They are basically a redifinition of the mappings with a reference to the original. Since we are redifing them we can change the definition and/or appearance. In general the layout/position of the imported elements is preserved which is what we are aiming for. 
...
feature:objects
```
This will always show the pattern element even when we have instances. However, that is not the behaviours we want. For better overview we want to hide the pattern object when an isntance object is present. We could redefine the *Semantic Candidate Expression* aobve as
```
...
We have four Relation mappings - instance to instance; pattern to pattern; instance to pattern; pattern to instace. The first one uses a solid line whereas the other three use dashed line (different style) as those relations are from the pattern and not instances themselves. It should be possible to combine the last three relations into one mapping, however, I decided to keep them separate and use simple model query expressions in the specification.

<img src="doc/img/instanceLayerSpec.jpg" /> <br/>
...
For the sake of illustration there are two tools for instantiating objects. The first is a node creation tool. It automatically creates a node view for the created semantic element. The second is a general purpose tool which in general does not create any views (but those can be explicitely created in the specification), however, when a mapping is synchronized (Adcanced Tab in the Properties view) views will be created and deleted by Sirius automatically to reflect the state of the model. That is why we can reuse (by means of copy&paste) the same definition for those two tools. There is a sligh difference though. The *Node Creation Tool* provides access to the semantic and graphical elements the user clicks on via the variables "container" and "containerView", while the general purpose tool uses "element" and "elementView". To make the definitions compatible we add an Expression Variable to "element" named "container" with the value of "element", effectivly creating an alias. The same is done respectively for "elementView".
...
<img src="doc/img/instantiateToolSpec.jpg" /> <br/>
...
In our case, we want to use the filter to provide a specific editor behavior, namely, hide pattern obejct views when an object is instantiated. For consistency we want this filter to be always active. There is no such option in Sirius yet. There is something called "Concenrs" which could be used to specify that a filter is active by default, but that does not prevent the user from disabling it.
...
An External Java Action class implements IExternalJavaAction. It must be registered in the Extenssions tab of plugin.xml. It can then be used by its ID in the specification of the diagram.
An important java action that already exists in Sirius is org.eclipse.sirius.business.api.action.printVariables which prints out to the console the avialbe variables and their values in a certain context. Invaluable for debugging.
